...
To understand better whether there are meaninful differences in tags used for videos related to mainstream media, for the rest of this section I differentiate between mutually exlusive groups: tags for videos related to mainstream media, and tags for videos unrelated to mainstream media. I subtract the tag counts of `ms_dat` from the `full_dat` group, because I previously was treating the two groups as inclusive. 
...
ggplot(all_tag_comps, aes(valClean, proportion, color = type, fill = type)) +
  ...
  labs(x = "Tag", y = "% of tag uses", title = "Tag Comparison", subtitle = "All tags") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
...
It is difficult to visually infer tags from the plot with a sizable difference in usage across the two video types. In this next plot, I highlight tags with an absolute difference of more than two percent between the two groups. The make this more concrete, what follows is the most extreme example we see in the data: the tag "Info Wars". This tag constitutes 3.7% of the tags in the mainstream media-related videos, and constitutes 12.5% of the tags in the non mainstream-media related videos. We see a difference of almost nine percentage points for this tag. Below is a plot presenting the share of tag use for tags with substantial variation in use between the two groups. 

```{r}
ggplot(var_tag_comps, aes(reorder(valClean, -proportion), proportion, color = type, fill = type)) +
  geom_bar(stat = "identity", position = "dodge") + 
  labs(x = "Tag", y = "% of tag uses", title = "Tag Comparison", subtitle = "Filter: tags with a non-negligible difference in use statistics") + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) + 
  scale_y_continuous(labels = scales::percent) + 
  theme(legend.position="bottom",
        legend.title=element_blank())
