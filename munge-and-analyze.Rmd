...
*view this analysis as a way to detect patterns and heed warnings for future flares in far-right extremism on YouTube*
^^ videos that are nominally about 
obligation to pay attention to the political crises reflected in this video content 
...
4. The variable `favoriteCount` doesn't contain any information.
...
I estimate a linear regressions to answer this question, controlling for a number of variables that I found earlier are associated with potential outcome variables. These include the number of days that each video was available online for viewing, and the number of tags associated with each video. I test a few different outcome variables: the number of video likes, the number of comments for each video, and the number of dislikes. I control for the number of views in each video as a baseline or offset.

In math, I fit the following model: 

$$ Y_i = \alpha + \beta x_{1} + \delta x_{2} + \nu x_3 + \phi x_4 + \epsilon_i.$$

* The outcome $Y_{i}$ is the number of comments/views/likes for video $i$;
* $\alpha$ represents an intercept term.
* The coefficient $\beta$ describes the association between the "mainstream media-related" status and the outcome variable. The term $x_1 = 1$ indicates that a video was related to the mainstream media. 
* The coefficient $\delta$ describes the association between access to the video online and the outcome variable. The term $x_{2}$ indicates the number of days a video had been publicly available for before May 2018.
* The coefficient $\nu$ describes the association between video tags and the outcome variable. The term $x_{3}$ indicates the number of tags for video $i$.
* The coefficient $\phi$ describes the association between the number of views and the outcome variable, acting as an offset. 
