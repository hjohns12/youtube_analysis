---
title: 'Mainstream Media Discourse by Far Right Influences on YouTube'
...
- set all fig.width and fig.height options to something adequately large

- make a thesis statement, prove/disprove it, and restate it a few times
...
# just a late-night thought for later but HEY why not just knit to an html and keep the md for later.... because seriously i think an html will do the trick
```
...
full_dat <- read_csv(file.path("~","Projects", "youtube_analysis", "data", "video_metadata.csv"))
```
...
ms_dat <- read_csv(file.path("~", "Projects", "youtube_analysis", "data", "video_metadata_mainstream_media.csv"))
...
4. `favoriteCount` doesn't provide any information.
...
cat_full_dat <- full_dat %>%
  ...
  mutate(main_stream = ifelse((main_stream_desc | main_stream_tags | main_stream_title), "Mainstream-related videos", "Non mainstream-related videos"),
         ...
         friendlyPublished = parse_date_time(cat_full_dat$publishedAt, orders = "Ymd HMS"),
         datePulled = parse_date_time("2018-05-24", orders = "Ymd"),
...
From the plot above, we see that videos published about the mainstream media were concentrated between December 2016 and August 2017. In the plot below, the x-axis represents the number of days each video was online from its time of publication and the date that I scraped it (May 24 2018).

```{r fig.align="center", warnings = FALSE, message=FALSE, error=FALSE}
ggplot(cat_full_dat) +
...
```{r ms_tag_count_plot, echo = FALSE, fig.align="center", fig.height=8, fig.width=15}
...
```{r all_tag_compare_fig, fig.align="center", warning=FALSE}
...
Question: Do videos related to mainstream media attract more attention than those unrelated to the mainstream media?

- Videos that were published earlier are associated with a higher number of views
  + I collected the data on Thursday, May 24, 2018. So length_available is the difference between this data and the date the videos was published (measured in number of days)
- A higher number of tags are associated with more views (??is this true??)

viewCount ~ factor(ms) + length_available + nTags
commentCount ~ factor(ms) + length_available + nTags
dislikeCount ~ factor(ms) + length_available + nTags
likeCount ~ factor(ms) + length_available + nTags
...
covars <- c("factor(ms)",
            "length_available",
            "nTags")
covar_formula <- paste0(covars, sep = " + ", collapse = "")

outcomes <- c("viewCount", "commentCount", "dislikeCount", "likeCount")
